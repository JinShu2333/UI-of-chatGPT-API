...
class ChatUI:
    ...
    def update_conversation(self, message, is_bot=False, new_line=True):
        if is_bot:
            message = f"Bot: {message}"
        else:
            message = f"You: {message}"
    ...
    def pause_chat(self):
        self.is_paused = not self.is_paused  # 切换状态
        if self.is_paused:
            self.update_conversation("Chatbot is paused. Type 'resume' to continue.", new_line=True)
        else:
            self.update_conversation("Chatbot is resumed.", new_line=True)
